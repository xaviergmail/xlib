...
local noglobals, getglobal, setglobal if pcall and not compat.lua52 and not release then
    ...
    local env = setmetatable({}, { __name="env", __index=errR, __newindex=errW })
    noglobals = function()
        pcall(setfenv, 3, env)
    end
...
util.load = function(a, b) return CompileString(a, b, false) end
...
function setmode(t,mode)
    ...
    mt.__name = mt.__name or "setmode"
    if mt.__mode then
...
local   load,   map,   map_all, t_pack
    = CompileString, u.map, u.map_all, u.pack
...
local hasCmt; hasCmt = setmetatable({}, { __name = "cmt", __mode = "k", __index = function(self, pt)
...
local   checkstring,   copy,   fold,   load,   map_fold,   map_foldr,   setify, t_pack, t_unpack
    = u.checkstring, u.copy, u.fold, CompileString, u.map_fold, u.map_foldr, u.setify, u.pack, u.unpack
local
...
if compat.proxies and not compat.lua52_len then
    ...
    local __index_LL = { __name = "indexll", __index = LL}
...
local function global(self, env) setmetatable(env,{ __name="global", __index = self}) end
...
local compat = {
    ...
    luajit = false,
    jit = false,
    lua52_len = not #setmetatable({},{ __name="l52len", __len = function()end}),
    proxies = pcall(function()
        local prox = newproxy(true)
        local prox2 = newproxy(true)
...
do
    ...
    IPv4address = Cg ( dec_octet * P"." * dec_octet * P"." * dec_octet * P"." * dec_octet ) / new_IPv4
...
local IPv6address_ = {}
do
    local unpack = table.unpack or unpack -- luacheck: ignore 113 143 local P = lpeg.P local V = lpeg.V local Cc = lpeg.Cc local Cg = lpeg.Cg
    ...
    IPv6address = raw_IPv6address / new_IPv6
